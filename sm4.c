...
static uint32_t l_prime_transform(uint32_t value) {
...
void sm4_set_encrypt_key(sm4_key *key, const uint8_t raw_key[16]) {
    ...
    k[2] = load_be32(raw_key + 8) ^ SM4_FK[2];
    k[3] = load_be32(raw_key + 12) ^ SM4_FK[3];
...
void sm4_set_decrypt_key(sm4_key *key, const uint8_t raw_key[16]) {
    sm4_key enc_key;
    sm4_set_encrypt_key(&enc_key, raw_key);
    for (size_t i = 0; i < 32; ++i) {
        key->rk[i] = enc_key.rk[31 - i];
    }
}
